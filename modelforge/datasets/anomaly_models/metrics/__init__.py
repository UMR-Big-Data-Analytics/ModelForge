...
- **Classification-metrics:** These metrics are defined over binary classification predictions (zeros or ones), thus
  they require a thresholding strategy to convert anomaly_models scorings to binary classification results.
...
- **Range-metrics:** Range-metrics compute the quality scores for anomaly_models ranges (windows) instead of each point in the
  time series.
...
class DefaultMetrics:
    """Default metrics of TimeEval that can be used directly for time series anomaly_models detection algorithms without
