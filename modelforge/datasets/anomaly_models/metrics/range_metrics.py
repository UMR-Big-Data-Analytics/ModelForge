...
class RangePrecision(Metric):
    ...
    Range precision is the average precision of each predicted anomaly_models range. For each predicted continuous anomaly_models
    range the overlap size, position, and cardinality is considered.

    Parameters
    ----------
    thresholding_strategy : ThresholdingStrategy
        Strategy used to find a threshold over continuous anomaly_models scores to get binary labels.
...
class RangeRecall(Metric):
    ...
    Range recall is the average recall of each real anomaly_models range. For each real anomaly_models range the overlap size,
    position, and cardinality with predicted anomaly_models ranges are considered. In addition, an existence reward can be
    given that boosts the recall even if just a single point of the real anomaly_models is in the predicted ranges.
    ...
    alpha : float
        ...
        given if the real anomaly_models range has overlap with even a single point of the predicted anomaly_models range.
    cardinality : {'reciprocal', 'one', 'udf_gamma'}
        Cardinality type.
    bias : {'flat', 'front', 'middle', 'back'}
        Positional bias type.
