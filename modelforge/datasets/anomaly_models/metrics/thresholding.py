...
class ThresholdingStrategy(ABC):
    """Takes an anomaly_models scoring and ground truth labels to compute and apply a threshold to the scoring.

    Subclasses of this abstract base class define different strategies to put a threshold over the anomaly_models scorings.
    ...
    def fit(self, y_true: np.ndarray, y_score: np.ndarray) -> None:
        ...
        y_score : np.ndarray
            Anomaly scoring with continuous anomaly_models scores (same length as `y_true`).
        """
        self.threshold = self.find_threshold(y_true, y_score)

    def transform(self, y_score: np.ndarray) -> np.ndarray:
        """Applies the threshold to the anomaly_models scoring and returns the corresponding binary labels.
    ...
    def transform(self, y_score: np.ndarray) -> np.ndarray:
        ...
        if y_score.dtype not in [np.int_, np.bool_]:
            raise ValueError(
                ...
                "Continuous anomaly_models scorings are not supported, please use any other thresholding "
                "strategy for this!"
            )
        return y_score
...
class FixedValueThresholding(ThresholdingStrategy):
    ...
    ----------
    threshold : float
        Fixed threshold to use. All anomaly_models scorings are scaled to the interval [0, 1]
    """
...
class PercentileThresholding(ThresholdingStrategy):
    """Use the xth-percentile of the anomaly_models scoring as threshold.

    Parameters
    ----------
    percentile : int
        The percentile of the anomaly_models scoring to use. Must be between 0 and 100.
    """
    ...
    def find_threshold(self, y_true: np.ndarray, y_score: np.ndarray) -> float:
        ...
        -------
        threshold : float
            The xth-percentile of the anomaly_models scoring as threshold.
        """
...
class TopKPointsThresholding(ThresholdingStrategy):
    ...
    ----------
    k : optional int
        Number of expected anomalous points. If `k` is `None`, the ground truth point is used to calculate the real
    ...
    def find_threshold(self, y_true: np.ndarray, y_score: np.ndarray) -> float:
        ...
        If `k` is `None`, the ground truth point is used to calculate the real ratio of anomalous points to all points.
...
class TopKRangesThresholding(ThresholdingStrategy):
    ...
    ----------
    k : optional int
        Number of expected anomalies. If `k` is `None`, the ground truth point is used to calculate the real number of
        anomalies.
    """
    ...
    def find_threshold(self, y_true: np.ndarray, y_score: np.ndarray) -> float:
        ...
        If `k` is `None`, the ground truth point is used to calculate the real number of anomalies (anomalous ranges).
...
class SigmaThresholding(ThresholdingStrategy):
    """Computes a threshold :math:`\\theta` based on the anomaly_models scoring's mean :math:`\\mu_s` and the
    ...
    def find_threshold(self, y_true: np.ndarray, y_score: np.ndarray) -> float:
        """Determines the mean and standard deviation ignoring NaNs of the anomaly_models scoring and computes the
...
class PyThreshThresholding(ThresholdingStrategy):
    ...
    threshold and to transform the continuous anomaly_models scoring into binary anomaly_models predictions.
...
def tmp_np_random_seed_pythresh(
    thresholder: "BaseThresholder", random_state: Any
) -> Generator[None, None, None]:  # type: ignore
