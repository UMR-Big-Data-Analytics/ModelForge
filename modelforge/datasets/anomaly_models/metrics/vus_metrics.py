...
class RangeAucMetric(Metric, ABC):
    ...
    def _extend_anomaly_labels(
        ...
        """Extends the anomaly_models labels with slopes on both ends. Makes the labels continuous instead of binary."""
        starts, ends = self.anomaly_bounds(y_true)

        if self._buffer_size is None:
            # per default: set buffer size as median anomaly_models length:
...
class RangePrAUC(RangeAucMetric):
    ...
    We first extend the anomaly_models labels by two slopes of ``buffer_size//2`` length on both sides of each anomaly_models,
    uniformly sample thresholds from the anomaly_models score, and then compute the confusion matrix for all thresholds.
    ...
    - Overlapping slopes don’t sum up in their anomaly_models weight, but we just take to maximum anomaly_models weight for each
    ...
    buffer_size : Optional[int]
        Size of the buffer region around an anomaly_models. We add an increasing slope of size ``buffer_size//2`` to the
...
class RangePrVUS(RangeAucMetric):
    ...
    For all buffer sizes from 0 to ``max_buffer_size``, we first extend the anomaly_models labels by two slopes of
    ``buffer_size//2`` length on both sides of each anomaly_models, uniformly sample thresholds from the anomaly_models score, and
    ...
    ----------
    max_buffer_size : int
        Maximum size of the buffer region around an anomaly_models. We iterate over all buffer sizes from 0 to
